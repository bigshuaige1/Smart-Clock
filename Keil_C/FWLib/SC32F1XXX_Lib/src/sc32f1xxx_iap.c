...
void IAP_EEPROMEraseSector ( uint32_t IAP_Sector )
{
    ...
    tmpreg = IAP->IAP_CON;
    tmpreg &= ( uint32_t ) ~ ( IAP_CON_ERASE | IAP_CON_CMD | IAP_CON_PRG );
    tmpreg |= ( uint32_t ) ( IAP_CON_SERASE  | ( 0x02 << IAP_CON_CMD_Pos ) );
    IAP->IAP_CON = tmpreg;
}
...
uint8_t IAP_ProgramHalfWordArray ( uint32_t Address, uint16_t* ByteArray, uint8_t ArraySize )
{
    assert_param ( IS_IAP_ADDRESS ( Address ) );
...
/**
  ...
  * @retval uint8_t :The length of the array that needs to be written.
  */
uint8_t IAP_ProgramByteArray ( uint32_t Address, uint8_t* ByteArray, uint8_t ArraySize )
{
    assert_param ( IS_IAP_ADDRESS ( Address ) );
...
/**
  ...
  * @retval uint8_t:The length of the array that needs to be written.
  */
uint8_t IAP_ReadWordArray ( uint32_t Address, uint32_t* ByteArray, uint8_t ArraySize )
{
    assert_param ( IS_IAP_ADDRESS ( Address ) );
...
/**
  ...
  */
uint8_t IAP_ReadHalfWordArray ( uint32_t Address, uint16_t* ByteArray, uint8_t ArraySize )
{
    assert_param ( IS_IAP_ADDRESS ( Address ) );
...
/**
  ...
  * @retval uint8_t:The length of the array that needs to be read.
  */
uint8_t IAP_ReadByteArray ( uint32_t Address, uint8_t* ByteArray, uint8_t ArraySize )
{
    assert_param ( IS_IAP_ADDRESS ( Address ) );
